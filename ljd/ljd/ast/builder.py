...
def _build_var_assignment(state, addr, instruction):
    ...
    if opcode == ins.MOV.opcode \
            ...
            or (ljd.CURRENT_VERSION > 20 and opcode == ins.ISTYPE.opcode) \
            or (ljd.CURRENT_VERSION > 20 and opcode == ins.ISNUM.opcode) \
...
def _build_unary_expression(state, addr, instruction):
    ...
    elif ljd.CURRENT_VERSION > 20 and opcode == ins.ISTYPE.opcode:
        operator.type = nodes.UnaryOperator.T_TOSTRING
    elif ljd.CURRENT_VERSION > 20 and opcode == ins.ISNUM.opcode:
